...
左侧“原型页面”按组显示树形层级，支持按页面名、`pageId`、入口路径和组名搜索，也可以单独收起列表分组。列表搜索与收起只影响左侧导航，Canvas 始终展示全部页面节点、页面组、连线和备注。组内页面 Tab 与局部自动布局预览仍可用于快速定位和调整；应用局部布局前会先备份 manifest，并且只更新当前组节点坐标。
...
顶部“播放原型”打开的右侧预览与公开 Share 页面都运行真实 iframe。页面自身的点击、输入、滚动、弹窗和 JavaScript 状态会原样执行；跨 manifest 页面跳转可使用 `data-protodock-page="<pageId>"`、`href="protodock:<pageId>"` 或 `protodock:navigate` 消息。旧静态页面只有在按钮文字与唯一出边标签明确一致时才会自动跳转，不会按按钮顺序猜测。
