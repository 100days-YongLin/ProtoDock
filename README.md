...
发布结果右侧还可以点击“发送到飞书机器人”。第一次使用时通过设置按钮填写飞书自定义机器人的 Webhook；ProtoDock 会把发布成功状态、更新内容、当前版本 PRD、持续最新版和 GitHub 分支组织为交互卡片。Webhook 配置写入项目根目录的 `protodock.local.json`，保存时自动加入 `.gitignore`，并且不会进入自动发布 ZIP、公开项目、下载包或 GitHub 推送。不要把 Webhook 写入 `protodock.project.json`、页面源码或产品文档；Webhook 泄露后应立即在飞书群机器人设置中重新生成。
