...
打开项目后，顶部的“完整产品文档”会在新标签页生成项目级 PRD 阅读视图。封面只读取正式发布的 `changelog`，展示日期、时间、版本号和合并后的更新内容；最新发布固定显示在变更历史顶部并高亮为当前版本，尚未发布的 `pendingChanges` 不会混入版本历史。正文按 `canvas.groups` 汇总业务模块；旧项目没有分组时自动按画布页面顺序展示。每个页面包含带设备框的完整长截图和对应 `docs/<page-id>.md` 正文，长截图会自动展开页面及内层滚动区域直至内容底部；支持目录定位、点击后滚动查看原尺寸大图，以及打印或导出 PDF。移动端和平板原型使用左侧原型、右侧 PRD；`web-landscape` 与 `web-portrait` 项目自动改为上方大幅原型、下方 PRD。可交互 Web 预览始终使用 manifest 声明的内部视口，文档中的浏览器壳只做等比缩放；截图也只会复用真实视口与项目设备规格完全一致的 iframe，否则会在独立标准视口中重新渲染，确保 Web 页面按 `1440 × 900` 等 manifest 尺寸计算布局。A4 横版打印再通过排版尺寸缩放原始截图，不改变截图分辨率。文档顶部的“返回流程图”会优先回到原 Canvas 标签；浏览器无法关闭文档标签时则在当前标签打开来源流程图。首次打开会并行生成截图并复用相同图片、字体和样式资源；截图完成后写入浏览器持久缓存，再次打开时直接读取。页面目录、公共素材、设备规格、安全区或截图模式变化后，对应缓存会自动失效。单页截图失败不会阻断其他 PRD 内容。
