...
- Agent 每次完成一批项目修改时，只向 `pendingChanges` 追加 ISO 8601 时间和变更内容，不自行填写版本号。变更内容按“用户体验 / 产品调整 / 前后端逻辑”分栏并使用短项目符号；前两栏必填，第三栏仅记录可感知的交互、接口、数据、权限或同步规则。ProtoDock 正式发布时把累计内容合并为一条 `changelog`，并以发布弹窗中的版本号为唯一版本；历史发布项不得回写或删除。
...
只要项目存在未保存修改，顶部保存按钮都会要求填写“用户体验”和“产品调整”，并可选填“前后端逻辑”，每行一项；ProtoDock 自动生成三栏短项目符号并追加到 `pendingChanges`。可以累计多次保存，期间不产生新版本。统一发布时必须填写发布版本与更新内容；发布前应合并同类项，生成一条简短的正式 `changelog`，成功后再写回本地并清空 `pendingChanges`。完整产品文档只展示正式发布历史，最后一条就是当前版本。旧项目没有这些字段、使用旧前缀或历史记录仍是散文格式时仍可打开。
