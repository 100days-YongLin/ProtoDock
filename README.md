...
打开项目后，顶部的“完整产品文档”会在新标签页生成项目级 PRD 阅读视图。它按 `canvas.groups` 汇总业务模块；旧项目没有分组时自动按画布页面顺序展示。每个页面包含带设备框的原型截图和对应 `docs/<page-id>.md` 正文，支持目录定位、点击查看大图以及打印或导出 PDF。截图会逐页生成，阅读页会同步显示进度；单页截图失败不会阻断其他 PRD 内容。
