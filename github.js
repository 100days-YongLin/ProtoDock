(() => {
  ...
  function renderConfig() {
    ...
    if (els.publicKey) {
      if (isLoadingConfig) {
        els.publicKey.textContent = '正在生成公钥...';
  ...
  function updateState() {
    ...
    if (els.copyKey) {
      els.copyKey.disabled = !config?.publicKey;
    }
    if (els.refresh) {
  ...
  async function loadConfig() {
    ...
    try {
      ...
      if (!config.configured) {
        setStatus('服务器还没有配置固定 GitHub 仓库，请先设置 PROTODOCK_GITHUB_REPO');
