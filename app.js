...
- 每次完成一批修改后，向 \`${MANIFEST_FILE}\` 的 \`pendingChanges\` 追加时间和变更内容，不要自行生成版本号。变更内容必须使用短项目符号：所有 \`- 用户：...\` 在前，所有 \`- 产品：...\` 在后；禁止整段技术叙述。ProtoDock 发布时会把累计内容合并为一条正式 \`changelog\`，并以发布版本号为准。
