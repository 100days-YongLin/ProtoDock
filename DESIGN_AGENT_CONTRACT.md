...
├── shared-docs/*.md
└── prototypes/<endpoint>/protodock.project.json
```
...
- Put product-wide role definitions, shared business rules, shared data meaning, and common terminology in direct Markdown files under `shared-docs/`. Keep endpoint-specific page behavior in that endpoint's `docs/<page-id>.md`; do not duplicate shared rules across every page document.
...
- Validate the workspace root and every final endpoint ZIP. A valid workspace never replaces final ZIP validation.
