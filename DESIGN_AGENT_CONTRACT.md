...
30. Every completed batch of Agent edits must append one item to top-level `pendingChanges` with ISO 8601 `changedAt` and a concise bullet-list `description`; it must not invent or reuse a release version. Write `- 用户：...` bullets first and `- 产品：...` bullets second, one outcome per line, with no engineering diary content. ProtoDock publishing merges all pending items into one append-only `changelog` entry whose version is exactly the publish version, then clears `pendingChanges`. Never rewrite, reorder, or delete earlier release history.
