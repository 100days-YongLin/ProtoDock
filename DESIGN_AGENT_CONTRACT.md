...
- `protodock.workspace.json` owns only product identity, the shared-docs path, product version, and endpoint paths. It must not contain pages, Canvas data, copied child manifests, feature graphs, or endpoint implementation settings.
- Each `projects[].path` must resolve inside the workspace and directly identify a complete ProtoDock project root. Existing endpoint manifests remain valid and independently openable.
- A new standalone workspace uses its root as the product's single Git root. If the product already lives in one same-product monorepo, retain that ancestor as the only Git root and use the workspace root as the product source boundary. Do not initialize nested repositories under endpoint projects. Cross-end changes must be reviewable in one branch and diff so shared contracts, endpoint PRDs, prototypes, and release notes cannot drift into separate histories.
- Root `README.md` states the product scope, endpoint list, documentation entry points, build commands, validation commands, and Git delivery workflow. Every endpoint README states that endpoint's responsibility, editable source, build mapping, and verification commands.
- Direct `shared-docs/*.md` files own product-wide truth. Use the standard documents for product scope, roles and permissions, domain/data, interface/events, cross-end flows, and decisions/open questions. Keep endpoint-specific presentation and interaction behavior in the owning `docs/<page-id>.md`.
- Every page PRD in a workspace contains `关联共享契约` with `共享契约`, `本页职责`, and `依赖能力`. Reference stable shared document and flow identifiers; do not copy permission matrices, state machines, field definitions, API contracts, or cross-end acceptance into each page.
- Opening the workspace changes navigation only. The Canvas always renders the complete current endpoint and must not merge, hide, or rewrite child Canvas state.
- Agents editing a shared rule must update one shared Markdown file. Agents editing endpoint behavior must update only the affected endpoint artifacts and page PRDs.
- A cross-end feature is contract-first: product/design defines roles, scope, state, endpoint responsibilities, exceptions, and end-to-end acceptance; backend defines source of truth, authorization, transitions, interface/event behavior, idempotency, failure recovery, audit, and integration fixtures; each frontend maps those contracts to its page states and user actions.
- Do not infer backend rules from screenshots or infer frontend states from an endpoint name. When a shared contract is missing, contradictory, or undecided, record the question in `90-decisions-open-questions.md` and stop the affected behavior instead of inventing a rule.
- A cross-end feature is complete only when the shared contract, every affected page PRD, executable prototype state, required Canvas increments, `pendingChanges`, workspace validation, endpoint validation, final ZIP validation, and player/share click acceptance agree in the same Git diff.
- Publishing packages only the current endpoint and a read-only copy of shared docs under `docs/_shared/`. `workspaceSnapshot` is release metadata generated by ProtoDock; Agents must not write it into editable child manifests.
- All endpoint releases belonging to one coordinated product release use the same `product.version`. Different endpoints publish under stable endpoint-specific identifiers so one endpoint cannot overwrite another.
- Validate the workspace root and every final endpoint ZIP. Workspace collaboration gaps are warnings for old-project compatibility; new workspaces and CI should use `--workspace-contracts-as-errors`. A valid workspace never replaces final ZIP validation.
...
52. Product workspace support is additive. A directory without `protodock.workspace.json` must retain the single-project open, edit, save, Canvas, document, preview, Share, Git publish, changelog, capture, presentation, and ZIP behavior. Never require a legacy project to migrate before it can be opened or released.
53. Opening a workspace or child project must not rewrite its directory, manifest, shared documents, Canvas, Git history, or release history. Collaboration warnings are migration guidance; only an explicit user-approved migration may add standard shared documents, move repositories, or rewrite references.
54. New multi-end workspaces use one product Git history and one coordinated product version. The Git root is the workspace root for a standalone product or the existing ancestor for a same-product monorepo. Endpoint projects remain independently valid artifacts, but cross-end changes must not be split into unrelated permanent branches or nested repositories that hide the complete product diff.
55. Shared contract authority is explicit: product scope and endpoint responsibilities live in `01-product-overview`; roles and access in `02-roles-permissions`; terms, entities, fields, states, ownership, and consistency in `03-domain-data-contract`; frontend/backend behavior in `04-interface-event-contract`; cross-end sequences and acceptance in `05-cross-end-flows`; unresolved decisions in `90-decisions-open-questions`.
56. Page PRDs in a workspace must name their shared contract and local responsibility. They may elaborate page behavior but must not redefine a shared role, field, state, permission, API outcome, synchronization rule, or cross-end acceptance. Conflicts are resolved in the shared contract before implementation continues.
