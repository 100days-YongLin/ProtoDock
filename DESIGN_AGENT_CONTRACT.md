...
- A new workspace uses its root as the product's single Git root. Do not initialize nested repositories under endpoint projects. Cross-end changes must be reviewable in one branch and diff so shared contracts, endpoint PRDs, prototypes, and release notes cannot drift into separate histories.
...
54. New multi-end workspaces use one product Git root and one coordinated product version. Endpoint projects remain independently valid artifacts, but cross-end changes must not be split into unrelated permanent branches or nested repositories that hide the complete product diff.
